...
import { type IWhyMeRepository, createWhyMeRepository } from '../shared/api/whyme/whymeRepo';

export const useWhyMeStore = defineStore('whyMe', {
  state: () => ({
    articles: [] as Artycle[],
    repository: null as IWhyMeRepository | null,
  }),
  actions: {
    init(useApi: boolean) {
      this.repository = createWhyMeRepository(useApi);
      this.fetchArticles();
    },
